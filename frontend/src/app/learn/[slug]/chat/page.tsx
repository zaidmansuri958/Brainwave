      ...
      <div className="bw-shell">
        <div className="bw-band mb-4 flex items-center gap-3 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-sm font-semibold text-gray-900">AI Tutor</h1>
            <p className="truncate text-xs text-gray-500">{course?.title || "Loading..."}</p>
          </div>
          <button type="button" onClick={clearHistory} className="rounded-lg p-2 text-slate-400 transition hover:bg-[#f8f2eb] hover:text-gray-900">
            <Trash2 className="h-4 w-4" />
          </button>
          <Link href={`/learn/${params.slug}/community`} className="hidden items-center gap-1.5 rounded-full border border-slate-200 px-3 py-1.5 text-xs text-gray-600 sm:flex">
            ...
            {messages.map((message, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className={`flex gap-3 ${message.role === "user" ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`flex max-w-[78%] flex-col ${message.role === "user" ? "items-end" : "items-start"}`}>
                  <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${message.role === "user" ? "rounded-tr-md bg-slate-950 text-white" : "rounded-tl-md border border-slate-200 bg-white text-gray-700"}`}>
                    {message.content ? (
                      message.role === "assistant" ? (
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            h2: ({ children }) => <h2 className="mb-1.5 mt-1 text-sm font-bold text-gray-900">{children}</h2>,
                            ...
                            blockquote: ({ children }) => <blockquote className="my-2 border-l-2 border-indigo-300 pl-3 italic text-gray-500">{children}</blockquote>,
                  ...
                  {message.sources?.length ? <div className="mt-1.5 flex items-center gap-1 text-[11px] text-gray-500"><BookOpen className="h-3 w-3" />{message.sources.length} passage{message.sources.length > 1 ? "s" : ""} from course material</div> : null}
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          {messages.length === 1 ? (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
              {SUGGESTIONS.map((suggestion) => (
                <button key={suggestion} type="button" onClick={() => sendMessage(suggestion)} className="rounded-xl border border-slate-200 px-4 py-3 text-left text-sm text-gray-600 transition hover:bg-white hover:text-gray-900">
                  {suggestion}
                </button>
              ))}
            </motion.div>
      ...
      <div className="bw-shell pb-6">
        <div className="bw-band mx-auto max-w-4xl p-4">
          <div className="flex items-end gap-3">
            <div className="relative flex-1">
              <textarea
                ...
                rows={1}
                className="max-h-32 w-full resize-none rounded-xl border border-slate-200 bg-[#fcf8f3] px-4 py-3 text-sm text-gray-900 outline-none transition placeholder:text-slate-400 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
          ...
          <p className="mt-2 text-center text-[11px] text-gray-500">
            ...
            <Link href={`/learn/${params.slug}/community`} className="text-gray-700 underline underline-offset-2">
              Ask the community
            </Link>{" "}
