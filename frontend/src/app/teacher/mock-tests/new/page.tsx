  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9]">
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        <Link href="/teacher/mock-tests" className="inline-flex items-center gap-2 text-gray-500 text-sm mb-6 hover:text-gray-800">
          <ArrowLeft className="h-4 w-4" /> Back
        </Link>
        <h1 className="font-display font-extrabold text-2xl text-gray-900 mb-6">New mock test package</h1>
        <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4 shadow-sm">
          ...
          >
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Create & build papers"}
          </button>
        </form>
      </div>
    </div>
