...
import {
  ...
  Package, Download, ArrowRight, Star,
} from "lucide-react";
...
const FILE_ICONS: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  default:  { icon: FileStack, color: "text-violet-600", bg: "bg-violet-50" },
};
...
export default function MaterialsCatalogPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-indigo-900 via-violet-950 to-slate-900 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-indigo-500/10 blur-3xl" />
          <div className="absolute bottom-0 left-1/3 w-64 h-64 rounded-full bg-violet-500/10 blur-2xl" />
        </div>
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-bold text-indigo-300 uppercase tracking-widest">Marketplace</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-3">Study Materials</h1>
          <p className="text-indigo-200 text-base max-w-xl mb-8">
            Notes, PDFs, and bundles from verified teachers — instant access after purchase.
          </p>
          {/* Search */}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
            ...
            <p className="text-base font-bold text-gray-900 mb-1">
              {search ? "No materials match your search" : "No study materials yet"}
            </p>
            <p className="text-sm text-gray-500">
              {search ? "Try a different keyword" : "Teachers are uploading materials — check back soon!"}
            </p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-6">
              <p className="text-sm text-gray-500">
                <strong className="text-gray-900">{items.length}</strong> product{items.length !== 1 ? "s" : ""} available
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {items.map((item: any, i: number) => {
                const typeKey = (item.primary_file_type || "default") as keyof typeof FILE_ICONS;
                const { icon: Icon, color, bg } = FILE_ICONS[typeKey] ?? FILE_ICONS.default;
                ...
                return (
                  <motion.div key={item.id}
                    ...
                    <Link href={`/catalog/materials/${item.slug}`}
                      ...
                      {/* Card top */}
                      <div className="bg-gradient-to-br from-slate-50 to-indigo-50/60 px-5 py-6 border-b border-gray-100">
                        <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${bg} border border-gray-100 shadow-sm mb-4 group-hover:scale-110 transition-transform`}>
                      ...
                      <div className="px-5 py-4 flex items-center justify-between mt-auto">
                        ...
                        <span className={`text-sm font-extrabold ${item.price === 0 ? "text-green-600" : "text-gray-900"}`}>
                          {fmtRupee(item.price)}
                        </span>
                      </div>
                    </Link>
